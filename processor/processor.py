    ...
    with mlflow.start_run(experiment_id=experiment_id, nested=True):
        ...
        for epoch in range(1, epochs + 1):
            ...
            for n_iter, (img, vid, target_cam, target_view) in enumerate(train_loader):
                ...
                mlflow.log_metric(key="train/accuracy", value=acc, step=steps)
                mlflow.log_metric(key="train/loss", value=loss.item(), step=steps)
                mlflow.log_metric(key="utils/time_per_batch", value=time_per_batch, step=steps)
